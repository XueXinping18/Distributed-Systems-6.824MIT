...
func (kv *KVServer) HandleOperation(args *OperationArgs, reply *OperationReply) {
	...
	if ok && cache.SeqNum == args.SeqNum {
		...
		reply.LeaderId, reply.TermId = -1, -1
	...
	if !success {
		// redirection
		reply.LeaderId, reply.TermId = indexOrLeader, term
		reply.Err = ErrWrongLeader
		kv.logRPC(false, args.ClerkId, args.SeqNum, "Attempt to communicate with a non-leader! "+
			"Suggested LeaderID %d in term %d", indexOrLeader, term)
		return
	}
	...
	if response.SeqNum != args.SeqNum || response.ClerkId != args.ClerkId || response.Err == ErrLogEntryErased {
		...
		reply.TermId, reply.LeaderId = -1, -1
		...
		reply.TermId, reply.LeaderId = term, kv.me
...
func (kv *KVServer) logRPC(fatal bool, clerkId int64, seqNum int, format string, args ...interface{}) {
	if Debug {
		clerkStr := int64ToBase64(clerkId)
